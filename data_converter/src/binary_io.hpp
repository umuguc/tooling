...
void write_grid_file_internal(const std::string& filename, const Grid& grid, const std::pair<int, int>& xy, const SizeRetriever& retrieve_size) {
	...
	for(size_t i = 0; i < xy.first; i++) {
		for(size_t j = 0; j < xy.second; j++) {
			...
			binaryFile.write(reinterpret_cast<char*>(&gridIndex), sizeof(detail::GridIndex));
			...
			for(size_t k = 0; k < retrieve_size(i, j); k++) {
				...
				binaryFile.write(reinterpret_cast<const char*>(&point), sizeof(DataTY) * 3);
			}
		}
	}
}
...
std::vector<std::vector<std::vector<Point>>> read_grid_file(const std::string& filename) {
	...
	while(!binaryFile.eof()) {
		int amount;
		detail::GridIndex gridIndex;
		binaryFile.read(reinterpret_cast<char*>(&gridIndex), sizeof(detail::GridIndex));
		binaryFile.read(reinterpret_cast<char*>(&amount),
		    sizeof(int)); // Read the closing parenthesis

		if(static_cast<size_t>(gridIndex.x) >= grid.size()) { grid.push_back(std::vector<std::vector<Point>>()); }

		if(static_cast<size_t>(gridIndex.y) >= grid[gridIndex.x].size()) { grid[gridIndex.x].push_back(std::vector<Point>()); }
...
void read_grid_file(const std::string& filename, PointGrid& grid, AmountGrid& amountGrid) {
	std::ifstream binaryFile(filename, std::ios::binary);
	if(!binaryFile) {
		std::cerr << "Cannot open the binary file for reading." << std::endl;
		return;
...
template <typename Point>
void write_point_file(const std::string& filename, const std::vector<Point>& points, std::ios_base::openmode mode = std::ios::binary) {
	std::ofstream binaryFile(filename, mode);
	if(!binaryFile) {
		std::cerr << "Cannot open the binary file for writing." << std::endl;
		return;
	}
...
std::vector<Point> read_point_file(const std::string& filename) {
	...
	Point point;
	while(binaryFile.read(reinterpret_cast<char*>(&point), sizeof(DataTY) * 3)) {
